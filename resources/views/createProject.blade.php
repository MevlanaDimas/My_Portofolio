...
<body class="bg-stone-800">
                ...
                <div class="flex items-center px-4">
                    ...
                    <nav id="nav-menu" class="hidden absolute py-5 bg-stone-800 shadow-lg rounded-lg max-w-[453px] w-full right-4 top-full lg:block lg:static lg:bg-transparent lg:max-w-full lg:shadow-none lg:rounded-none">
                        <ul class="block lg:flex">
                            ...
                            <li class="group">
                                <a href="/projects" class="text-base text-par py-2 mx-8 flex group-hover:text-secondary">About Me</a>
                            </li>
                            <li>
        ...
        <div class="container place-content-center">
            <div class="flex flex-wrap">
                <div class="w-full self-center px-4">
                    <h1 class="pb-5 block font-bold text-primary text-center items-center justify-center text-4xl mt-1 lg:text-5xl">Create Project</h1>
                </div>
            </div>
        </div>
    </section>
    ...
    <section class="pt-36 pb-16 bg-gray-200">
        <div class="container w-full lg:w-2/3 lg:mx-auto">
            <form action="{{ url('/projects/storeProject') }}" method="post" enctype="multipart/form-data">
                @csrf
                <div class="w-full px-4 mb-8">
                    <label for="img_file" class="ml-0 text-sm font-bold text-button">Picture</label>
                    <input type="file" name="img_file" id="img_file" value="{{ old('img_file') }}" required class="w-full mt-2 bg-stone-400 text-base text-center text-gray-950 p-3 rounded-md focus:outline-none focus:ring-gray-50 focus:ring-1 focus:border-rose-350"/>
                </div>
                <div class="w-full px-4 mb-8">
                    <label for="title" class="ml-0 text-sm font-bold text-button">Title</label>
                    <input type="text" name="title" id="title" value="{{ old('title') }}" required class="w-full mt-2 bg-stone-400 text-gray-950 text-base p-3 rounded-md focus:outline-none focus:ring-gray-50 focus:ring-1 focus:border-rose-350"/>
                </div>
                <div class="w-full px-4 mb-8">
                    <label for="proj_desc" class="ml-0 text-sm font-bold text-button">Description</label>
                    <textarea name="proj_desc" id="proj_desc" class="w-full mt-2 bg-stone-400 text-gray-950 text-base p-3 rounded-md focus:outline-none focus:ring-gray-50 focus:ring-1 focus:border-rose-350 h-50">{{ old('proj_desc') }}</textarea>
                </div>
                <div class="w-full px-4 mb-8">
                    <label for="web" class="ml-0 text-sm font-bold text-button">Link Website</label>
                    <input type="url" name="web" id="web" value="{{ old('web') }}" class="w-full mt-2 bg-stone-400 text-gray-950 text-base p-3 rounded-md focus:outline-none focus:ring-gray-50 focus:ring-1 focus:rose-350">
                </div>
                <div class="w-full px-4 mb-8">
                    <label for="github" class="ml-0 text-sm font-bold text-button">Github</label>
                    <input type="url" name="github" id="github" value="{{ old('github') }}" class="w-full mt-2 bg-stone-400 text-gray-950 text-base p-3 rounded-md focus:outline-none focus:ring-gray-50 focus:ring-1 focus:rose-350">
                </div>
                @if($errors->any())
