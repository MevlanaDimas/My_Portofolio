...
<body class="bg-stone-800">
        ...
        <div class="container place-content-center">
            <div class="flex flex-wrap">
                <div class="w-full self-center px-4">
                    <h1 class="pb-5 block font-bold text-primary text-center items-center justify-center text-4xl mt-1 lg:text-5xl">Create About Me</h1>
                </div>
            </div>
        </div>
    </section>
    ...
    <section class="pt-36 pb-16 bg-gray-200">
        <div class="container w-full lg:w-2/3 lg:mx-auto">
            <form action="{{ url('/about/storeAbout') }}" method="post" enctype="multipart/form-data">
                @csrf
                <div class="w-full px-4 mb-8">
                    <label for="photo_file" class="ml-0 text-sm font-bold text-button">Photo</label>
                    <input type="file" name="photo_file" id="photo_file" value="{{ old('photo_file') }}" reqiured class="w-full mt-2 bg-stone-400 text-base text-center text-gray-950 p-3 rounded-md focus:outline-none focus:ring-gray-50 focus:ring-1 focus:border-rose-350"/>
                </div>
                <div class="w-full px-4 mb-8">
                    <label for="bio" class="ml-0 text-sm font-bold text-button">Bio</label>
                    <textarea name="bio" id="bio" required class="w-full mt-2 bg-stone-400 text-gray-950 text-base p-3 rounded-md focus:outline-none focus:ring-gray-50 focus:ring-1 focus:border-rose-350 h-50">{{ old('bio') }}</textarea>
                </div>
                @if($errors->any())
